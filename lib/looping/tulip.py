...
try:
    ...
    # Use add-on package on Python2.x
    import futures
...
class PollPollster(PollsterBase):
    ...
    def __init__(self):
        super(PollPollster, self).__init__()
    ...
    def register_reader(self, fd, callback, *args):
        super(PollPollster, self).register_reader(fd, callback, *args)
        self._update(fd)

    def register_writer(self, fd, callback, *args):
        super(PollPollster, self).register_writer(fd, callback, *args)
        self._update(fd)

    def unregister_reader(self, fd):
        super(PollPollster, self).unregister_reader(fd)
        self._update(fd)

    def unregister_writer(self, fd):
        super(PollPollster, self).unregister_writer(fd)
        self._update(fd)
...
class EPollPollster(PollsterBase):
    ...
    def __init__(self):
        super(EPollPollster, self).__init__()
    ...
    def register_reader(self, fd, callback, *args):
        super(EPollPollster, self).register_reader(fd, callback, *args)
        self._update(fd)

    def register_writer(self, fd, callback, *args):
        super(EPollPollster, self).register_writer(fd, callback, *args)
        self._update(fd)

    def unregister_reader(self, fd):
        super(EPollPollster, self).unregister_reader(fd)
        self._update(fd)

    def unregister_writer(self, fd):
        super(EPollPollster, self).unregister_writer(fd)
        self._update(fd)
...
class KqueuePollster(PollsterBase):
    ...
    def __init__(self):
        super(KqueuePollster, self).__init__()
    ...
    def register_reader(self, fd, callback, *args):
        ...
        return super(KqueuePollster, self).register_reader(fd, callback, *args)

    def register_writer(self, fd, callback, *args):
        if fd not in self.writers:
            kev = select.kevent(fd, select.KQ_FILTER_WRITE, select.KQ_EV_ADD)
            self._kqueue.control([kev], 0, 0)
        return super(KqueuePollster, self).register_writer(fd, callback, *args)

    def unregister_reader(self, fd):
        super(KqueuePollster, self).unregister_reader(fd)
        kev = select.kevent(fd, select.KQ_FILTER_READ, select.KQ_EV_DELETE)
        self._kqueue.control([kev], 0, 0)

    def unregister_writer(self, fd):
        super(KqueuePollster, self).unregister_writer(fd)
...
class DelayedCall(object):
    ...
    def __init__(self, when, repeat, callback, args, kwds=None):
        self.when = when
        self.repeat = repeat
...
class EventLoop(object):
    ...
    def add_reader(self, fd, callback, *args):
        ...
        dcall = DelayedCall(None, None, callback, args)
    ...
    def call_soon(self, repeat, callback, *args):
        ...
        """
        dcall = DelayedCall(None, repeat, callback, args)
        self.ready.append(dcall)
        return dcall
    ...
    def call_later(self, when, repeat, callback, *args):
        ...
        if when < 10000000:
            when += time.time()
        dcall = DelayedCall(when, repeat, callback, args)
        heapq.heappush(self.scheduled, dcall)
        return dcall
    ...
    def run_once(self, timeout):
        ...
        if self.pollster.pollable():
            if self.scheduled:
                ...
                secs = max(0, when - time.time())
            else:
                secs = None
            if timeout is not None:
                secs = timeout if secs is None else min(secs, timeout)
            t0 = time.time()
            events = self.pollster.poll(secs)
            t1 = time.time()
        ...
        now = time.time()  # prevent infinite loop for very small repeats
        while self.scheduled:
            ...
            if dcall.repeat:
                dcall.when = now + repeat
                heapq.heappush(dcall)
